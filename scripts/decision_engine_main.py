#!/usr/bin/env python3
"""
FCDM Decision Engine (minimal real-AWS patch)

This version is intentionally kept very close to the old mock version.

What changed:
- CloudA is still controlled by OpenStack CLI
- CloudB is controlled by AWS CLI using a real EC2 instance ID

What did NOT change:
- config loading style
- forecast retrieval
- forecast reading
- required_units logic
- pricing logic
- decision logic
- logging structure
"""
...
CLOUDB_MAIN = _C["cloudb_main"]   # keep this as display label

# NEW: real AWS CloudB fields
CLOUDB_INSTANCE_ID = _C["cloudb_instance_id"]
CLOUDB_REGION      = _C["cloudb_region"]
...
def _append_decision_log(
    ...
    # small cleanup: use current labels
    chosen_name = CLOUDA_MAIN if choice == "A" else CLOUDB_MAIN
...
# ================================================================
# NEW AWS CLI HELPERS (only for CloudB)
# ================================================================

def _aws_cmd(args: list[str]) -> str:
    cmd = ["aws", "ec2", "--region", CLOUDB_REGION] + args
    p = subprocess.run(cmd, capture_output=True, text=True)
    if p.returncode != 0:
        raise RuntimeError((p.stderr or p.stdout).strip())
    return (p.stdout or "").strip()
...
def aws_status() -> str:
    out = _aws_cmd([
        "describe-instances",
        "--instance-ids", CLOUDB_INSTANCE_ID,
        "--query", "Reservations[0].Instances[0].State.Name",
        "--output", "text",
    ]).strip()

    # map AWS names to the same style you already use
    if out == "running":
        return "ACTIVE"
    if out == "stopped":
        return "SHUTOFF"
...
def aws_start() -> None:
    print(f"Starting {CLOUDB_MAIN}")
    if DRY_RUN:
        return
    _aws_cmd(["start-instances", "--instance-ids", CLOUDB_INSTANCE_ID])


def aws_stop() -> None:
    print(f"Stopping {CLOUDB_MAIN}")
    if DRY_RUN:
        return
    _aws_cmd(["stop-instances", "--instance-ids", CLOUDB_INSTANCE_ID])


def wait_for_aws_status(desired: str = "ACTIVE", timeout_sec: int = 300) -> bool:
    if DRY_RUN:
        return True
    start = time.time()
    while True:
        ...
        if st == desired:
            return True
        if time.time() - start > timeout_sec:
            print(f"WARNING: Timeout waiting for AWS → {desired} (current={st})")
            return False
        time.sleep(5)
...
def _decide(os_total: float, aws_total: float, a_st: str, b_st: str) -> tuple[str, str]:
    ...
    if aws_total < os_total:
        return "B", f"AWS cheaper → {CLOUDB_MAIN}"
    ...
    if b_st == "ACTIVE" and a_st != "ACTIVE":
        return "B", f"Equal cost → keep active {CLOUDB_MAIN}"
